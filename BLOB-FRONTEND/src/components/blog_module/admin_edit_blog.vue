...
export default {
    ...
    async mounted() {
      ...
      if (!access_token) {
        ...
        try {
          const r = await axios.delete("https://blob-backend.onrender.com//api/user_check_permission", null, {
            headers: {
              Authorization: `Bearer ${access_token}`
            }
          });
          if (r.status === 200) {
            const response=await axios.get(`https://blob-backend.onrender.com//api/getInfoBlog/${this.$route.params.id}`);
    ...
    methods:{
        ...
        async edit_blog(){
            try{
                const response=await axios.post(`https://blob-backend.onrender.com//api/edit_blob/${this.$route.params.id}`,
                    JSON.stringify({
                    title:this.name,
                    subtitle:this.title,
                    body:this.body,
