...
export default {
    ...
    async mounted() {
      ...
      if (!access_token) {
        ...
        try {
          ...
          if (r.status === 200) {
            const response=await axios.get(`https://blob-backend.onrender.com/api/getAdminInfoBlog/${this.$route.params.id}`);
    ...
    methods:{
        ...
        async edit_blog(){
            try{
                const response=await axios.put(`https://blob-backend.onrender.com/api/edit_admin_blob/${this.$route.params.id}`,
                    JSON.stringify({
                    title:this.name,
                    subtitle:this.title,
                    body:this.body,
