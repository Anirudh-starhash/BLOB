...
export default {
    ...
    async mounted() {
      ...
      if (!access_token) {
        ...
        try {
          ...
          if (r.status === 200) {
            const response=await axios.get(`https://blob-backend.onrender.com/api/getInfoReview/${this.$route.params.id}`);
    ...
    methods:{
        ...
        async edit_blog(){
            try{
                this.id=JSON.parse(localStorage.getItem("info")).id
                const response=await axios.put(`https://blob-backend.onrender.com/api/edit_review/${this.$route.params.id}/${this.id}`,
                    JSON.stringify({
                    title:this.name,
                    rating:this.title,
